...
function HexagonBackground({
  ...
  const updateGridDimensions = React.useCallback(() => {
    ...
    // Cap cell count so a 4K viewport does not spawn a 500+ node grid.
    const rows = Math.min(12, Math.ceil(window.innerHeight / rowSpacing));
    const columns = Math.min(14, Math.ceil(window.innerWidth / hexagonWidth) + 1);
